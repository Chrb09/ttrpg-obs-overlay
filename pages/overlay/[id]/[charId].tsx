...
function OverlayCharacter() {
  const router = useRouter();
  const { id, charId } = router.query;
  ...
  // Encontra o personagem específico com base no `charId`
  const selectedCharacter = data.characters.find((char) => char.id === parseInt(charId as string));

  if (!selectedCharacter) return <div>Personagem não encontrado.</div>;

  // Cria um array com apenas o personagem selecionado para reutilizar o `map` abaixo
  const charactersToRender = [selectedCharacter];
...
export default OverlayCharacter;
